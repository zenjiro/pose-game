...
|----------|------|--------|------------|------|------|
| Outline 削減 | 8方向→4方向 (上下左右) | Low | 20–35% | High | 品質許容なら段階1 |
...
## 優先実行ステップ (Phase)
### Phase 1 (即日 / 低リスク)
1. 8方向→4方向アウトラインへ切替 (拡張オプション保持 `--hud-outline-mode=4|8`)
...
|---------|------|------|----------|----------|
| T1 | アウトライン方向 8→4 切替 | - | フラグで 4/8 変更可 | draw_osd 平均 -15% |
...
| リスク | 影響 | 緩和策 |
|--------|------|--------|
| 文字にじみ (4方向/キャッシュ) | 視認性低下 | フラグで8方向戻し / SDF移行検討 |
